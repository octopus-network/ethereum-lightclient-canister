...
pub struct CheckpointFallback {
    /// Services Map
    pub services: HashMap<Network, Vec<CheckpointFallbackService>>,
...
impl CheckpointFallback {
    ...
    pub fn get_all_fallback_endpoints(&self, network: &Network) -> Vec<String> {
    ...
    pub fn get_healthy_fallback_endpoints(&self, network: &Network) -> Vec<String> {
        self.services[network]
            .iter()
